...
def classes_found(raw, cf_raster_crop, gt_raster_crop):
    """
    ...
    labelled_gt = label(gt_raster_crop)
    
    numBuildings = len(np.unique(labelled_gt))
    
    buildingsFound = 0
    for seg in tqdm(range(len(np.unique(labelled_gt)))):
        region = np.where(labelled_gt == seg)
        if np.isin(cf_raster_crop[region], 255).any():
            buildingsFound += 1 
            
    print("Found {} buildings out of {}".format(buildingsFound, numBuildings))
    
#    proportionFound = buildingsFound / numBuildings
    
    raw['Number of Buildings Present'] = numBuildings
    raw['Number of Buildings Found'] = buildingsFound
